...
var HighestZoomLevel = 16; // Enter the highest zoom level we are creating tiles for (should be less than OrgZoom; technically the  script should be able to handle values larger as well, but your image quality will suffer)
...
var FolderPath = "~/Desktop/map-tiles/";  //<-- path to where we will save our tiles
...
var OrgZoomLevel = 16;   // the Google Maps zoom level of your Photoshop document (for best results, you will need to resize your  Photoshop document to match a zoom level as closely as possible before running this script)
	...
	{            
		...
		if (!visibleLayersEmpty(currentDocument))
		{
			...
			if (saveGIF)
			{
				...
				saveFile = new File(FolderPath + "Zoom-" + ZoomLevel + "/" + ZoomLevel + "-" + TileX + "-" + TileY + ".gif");    
				//Set save options
			...
			{
				//Set path to file and file name
				saveFile = new File(FolderPath + "Zoom-" + ZoomLevel + "/" + ZoomLevel + "-" + TileX + "-" + TileY + ".png");    
				pngSaveOptions = new PNGSaveOptions();
				pngSaveOptions.interlaced = 0;
